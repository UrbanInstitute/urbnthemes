...
R is a powerful, open-source programming language and environment. R excels at data management and munging, traditional statistical analysis, machine learning, and reproducible research, but it is probably best known for its graphics. This .html file made with [R Markdown](http://rmarkdown.rstudio.com/) contains examples and instructions for popular and lesser-known plotting techniques in R. Furthermore, it includes instructions on how to leverage `library(urbnthemes)` theme which will create near-publication-ready plots. If you have any questions, please don't hesitate to contact Aaron Williams (awilliams@urban.org) or Kyle Ueyama (kueyama@urban.org). 

#### library(urbnthemes)

`library(urbnthemes)` is an R package created by Aaron Williams, Kyle Ueyama, and Ben Chartoff that makes `ggplot2` output align more closely with [the Urban Institute's Data Visualization style guide](http://urbaninstitute.github.io/graphics-styleguide/).

Follow the instructions in the [`library(urbnthemes)` GitHub repository](https://github.com/UI-Research/urbnthemes) to install the package. This only needs to be done once for each computer for each release of the package. 

Each session that uses `library(urbnthemes)` should start with:
...
* `library(urbnthemes)` does not produce publication ready graphics. Visual styles must still be edited using your project/paper's normal editing workflow.
...
```
install.packages(c("tidyverse", "ggrepel", "RColorBrewer", "extrafont"))
library(extrafont)
font_import()
loadfonts()
```
...
ggplot(data = diamonds, mapping = aes(clarity, fill = cut)) + 
  geom_bar(position = "dodge", width = 2 / 3) +
...
todo(aaron): only use size with a continuous variable
...
todo(aaron): replace this terrible heat map
...
todo(aaron): replace these terrible small multiples
